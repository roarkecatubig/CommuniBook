$(document).ready(function () {
    ...
    $(document).on("click", ".cancel-delivery-request", cancelDeliveryRequest);
    $(document).on("click", ".cancel-delivery-offer", cancelDeliveryRequest);
...
*/
    ...
    function displayPending(results_list) {
      ...
      if (results_list[i].postStatus === "PENDING") {
        var cancelLink = $("<a>")
        cancelLink.addClass("cancel-delivery-request");
      ...
      var cancelLink = $("<a>")
      cancelLink.addClass("cancel-delivery-offer");
      ...
      cancelLink.data("book", dataObj);
            ...
            var requestLink = $("<a>")
            requestLink.addClass("remove-dashboard");
            requestLink.text("Remove from dashboard");
            requestLink.data("book", dataObj)
          }
  ...
  if (results_list[i].UserId != loggedUser[0].id) {
    ...
    function cancelOffer() {
      ...
      $.ajax({
        method: "PUT",
        url: "/book/offer/update/" + selectedBook.id + "/DECLINED"
      })
        .then(function () {
          window.location.href = "/home";
        });
